...
if __name__ == "__main__":
    ...
    vensimModel.levers = [RealParameter(f"Proposed harvesting quota {t}", 0, 2) for t in range(45)] #
    ...
    with MultiprocessingEvaluator(vensimModel) as evaluator:
        results, convergence = evaluator.optimize(nfe=1000000, searchover='levers', convergence=convergence_metrics,
                                      epsilons=[0.0001,] * len(vensimModel.outcomes) , Scenario=ref_scen)


    results.to_excel('./Data/results_convergence7_supercomp.xlsx')
    convergence.to_excel('./Data/conv_convergence7_supercomp.xlsx')
